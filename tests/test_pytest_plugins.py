...
class RobotInitFailed(wpilib.TimedRobot):
    def robotInit(self):
        assert False
...
def _configure_pyfrc_plugin(pytester, robot_class="DummyRobot"):
...
from pyfrc.test_support.pytest_isolated_tests_plugin import IsolatedTestsPlugin
...
def test_pyfrc_plugin_success(pytester):
    _make_robot_module(pytester)
    _configure_pyfrc_plugin(pytester)
...
def test_pyfrc_plugin_failure_shows_output(pytester):
    _make_robot_module(pytester)
    _configure_pyfrc_plugin(pytester)
    pytester.makepyfile(test_failure="""
def test_robot_failure(robot):
    print("pyfrc failure output")
    assert False
""")
...
def test_builtin_pyfrc_tests_module(pytester, isolated):
    _make_robot_module(pytester)
    if isolated:
        _configure_isolated_plugin(pytester, robot_class="DummyRobot")
    else:
        _configure_pyfrc_plugin(pytester, robot_class="DummyRobot")
    pytester.makepyfile(pyfrc_test="from pyfrc.tests import *\n")
...
@pytest.mark.parametrize("isolated", [False, True])
@pytest.mark.parametrize("robot_class", _ROBOT_INIT_FAILURES)
def test_robot_init_failure_detection(pytester, isolated, robot_class):
    result = _run_robot_suite(
        pytester,
        isolated,
        robot_class,
        """
def test_robot_init_failure(robot, control):
...
def test_state_transitions(robot, control):
    ...
    for name in {expected!r}:
        assert getattr(robot, name, False)
""",
        "-vv",
    )
